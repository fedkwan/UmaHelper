...
def in_which_page(screen: np.array, ocr: ddddocr.DdddOcr, p_ocr: PaddleOCR):
    ...
    # 如果是【游戏登录后】的首页
    # 点进培育
    if np.all(screen[1261, 360] == np.array([247, 159, 28])) and np.all(
        screen[920, 650] == np.array([102, 68, 221])
    ):
        return "app_main"
    ...
    if (
        np.all(screen[1016, 316] == brown)
        or np.all(screen[1016, 345] == brown)
        or np.all(screen[1016, 375] == brown)
        or np.all(screen[1016, 404] == brown)
    ):
    ...
    fans_require_image = cv2.imread(
        ROOT_DIR + "/resource/cultivate/find/train_end_title.png"
    )
    handler = ImageHandler()
    fans_require_match = handler.is_sub_image_in_box(
        fans_require_image, screen, 292, 428, 394, 434
    )
    if fans_require_match:
