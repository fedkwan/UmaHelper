...
class Competition:
    def __init__(self, d: u2.connect, ocr: ddddocr.DdddOcr, setting_file="setting_1"):
    ...
    def competition(self):
        ...
        while True:
            ...
            if (
                    np.all(screen[950, 320] == gray)
                    and np.all(screen[950, 400] == gray)
                    and np.all(screen[1106, 320] == gray)
                    and np.all(screen[1106, 400] == gray)
            ):

                # 判断是不是每周结算时刻
                current_time = datetime.datetime.now()
                is_weekend_midnight = (
                        current_time.weekday() == 0 and current_time.hour < 5
                )

                # 判断第一格RP点槽是否为灰色，如果不是，说明还有RP点
                # 如果有RP点，也不是结算时刻，那就先进行队伍竞技场
                if (
                        np.all(screen[65, 430] != np.array([89, 76, 81]))
                        and is_weekend_midnight is False
                ):
            ...
            # 判断是不是每周结算时刻，是的话返回
            if np.all(screen[875, 360] == weekend_gray) and np.all(
                    screen[1015, 360] == gray
            ):
            ...
            if np.all(screen[875, 360] == gray) and np.all(screen[1015, 360] == gray):
                # 判断第一格RP点槽是否为灰色，如果不是，说明还有RP点
                # 如果有RP点，也不是结算时刻，那就开始挑选对手（跟上面重复的，再一次判定
            ...
            if (
                    np.all(screen[476, 360] == opponent_gray)
                    and np.all(screen[746, 360] == opponent_gray)
                    and np.all(screen[1016, 360] == opponent_gray)
            ):
                # 默认选第3个对手
            ...
            if np.all(screen[1030, 320] == gray) and np.all(screen[1030, 400] == gray):
                ...
                daily_competition_left_times_text = handler.get_text_from_image(
                    self.ocr, cropped_image
                )
                if (
                        daily_competition_left_times_text == "3"
                        or daily_competition_left_times_text == "6"
                ):
                ...
                handler = ImageHandler()
                daily_legend_competition_left_times_text = handler.get_text_from_image(
                    self.ocr, cropped_image
                )
                ...
                if (
                        daily_competition_left_times_text != "3"
                        and daily_competition_left_times_text != "6"
                ) and daily_legend_competition_left_times_text != "1":
                    self.d.click(360, 1220)  # 回到首页
                    print("竞赛结束")
                    time.sleep(DEFAULT_SLEEP_TIME)
                    
                    array = np.array(find_list)
                    np.savetxt('find_list.txt', array, fmt='%s', delimiter=',')
                    
                    array = np.array(click_list)
                    np.savetxt('click_list.txt', array, fmt='%s', delimiter=',')
                    
                    break

            # 如果是【每日竞赛】，选择月光赏或者木星杯的页面
            if np.all(screen[631, 150] == np.array([199, 179, 154])) and np.all(
                    screen[774, 150] == np.array([196, 176, 151])
            ):
                cropped_image = screen[11:37, 610:690]
                handler = ImageHandler()
                daily_competition_right_top_left_times_text = (
                    handler.get_text_from_image(self.ocr, cropped_image)
                )
                # 判断是否还有次数，没有次数的话就返回 （这里采用更复杂的逻辑，避免出现循环识图的情况）
                if "0" not in daily_competition_right_top_left_times_text:
                    # 默认都是选木星杯，打经验，钱一般用不完
            ...
            if np.all(screen[638, 50] == np.array([102, 153, 34])) and np.all(
                    screen[638, 240] == np.array([102, 153, 34])
            ):
                cropped_image = screen[11:37, 610:690]
                handler = ImageHandler()
                daily_competition_right_top_left_times_text = (
            ...
            # 如果在【每日传奇竞赛】选择马娘的页面
            if np.all(screen[430, 360] == np.array([60, 205, 101])) and np.all(
                    screen[460, 360] == np.array([106, 193, 129])
            ):
                # 向下滑动，直到找到目标马娘为止
                sub_image = cv2.imread(
                    self.dir
                    + "/uma_icon/"
                    + self.setting_dic["daily_legend_competition_opponent_uma"]
                    + ".png"
                )
                handler = ImageHandler()
